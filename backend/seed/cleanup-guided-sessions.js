/**
 * پاکسازی سطرهای تکراری guided_sessions.
 *
 * الگو: cleanup-wellness-moves.js (پرامپت ۰۲) — همان قراردادها.
 * اگر قبلاً چند بار seed اجرا شده، ممکن است سطرهای تکراری وجود داشته باشد
 * که مانع update می‌شود. این اسکریپت همه‌ی سطرها را با همان slug پیدا می‌کند
 * و فقط اولی را نگه می‌دارد.
 *
 * اجرا: node backend/seed/cleanup-guided-sessions.js
 */
...
async function main() {
    ...
    console.log(`\n✅ پاکسازی تمام شد: ${deleted} سطر تکراری حذف شد`);
}
